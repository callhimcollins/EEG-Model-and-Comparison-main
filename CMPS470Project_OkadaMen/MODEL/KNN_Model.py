...
for channel in channels:
    ...
    #Save so we can test
    with open(f'Model/Parameters/KNNParam_{channel}.pkl', 'wb') as f: pickle.dump(knn, f)

#Test Using Pre-Trained Model
#-------------------------------------------------------------------------------------------------------------------------------------

    with open(f'Model/Parameters/KNNParam_{channel}.pkl', 'rb') as f: knn_loaded = pickle.load(f)

    # Evaluate the model on the first test set
    print('Evaluate on Test Data')
    test_accuracy = knn_loaded.score(test_X, test_Y)
    ...
    test2_accuracy = knn_loaded.score(test2_X, test2_Y)
    print(f"Test{channel} accuracy:", test2_accuracy)

    #Generating Predictions
    predictions = knn_loaded.predict(test2_X)
